...
module.exports = {
  ...
  validate: (entity, schema, url) => {
    return (req, res, next) => {
      console.log(schema);
      const {error} = schema.validate(req[entity]);
  ...
  validateLogin: (req, res, next) => {
    ...
    const message = error.details[0].message;
    errorLogger(message);
    req.flash("loginErr", message);
    return res.redirect("/auth/login#login");
  },
  validateRegister: (req, res, next) => {
    const {error} = register.validate(req.body);
    if (!error) return next();
    // todo remove different auth errors
    const message = error.details[0].message;
    errorLogger(message);
    req.flash("registerErr", message);
    return res.redirect("/auth/login#register");
  },
  validateAddSong: (req, res, next) => {
    const {error} = song.validate(req.body);
    if (!error) return next();
  ...
  validateEditSong: (req, res, next) => {
    ...
    const {error: bodyError} = song.validate(req.body);
    const errors = [];
